...
- `yjs` `13.6.31`, MIT: provides the CRDT document, state vectors, update encoding, update application, and relative positions used by Docmost `/collab`.
...
- `@tiptap/pm` `3.27.1`, MIT: provides the ProseMirror schema/model primitives needed by `y-prosemirror`; version aligned with Docmost web's Tiptap stack.
- `esbuild` `0.28.1`, MIT: build-time bundler only. It creates the single JavaScript file that iOS loads through JavaScriptCore.

See `../../THIRD_PARTY_NOTICES.md` for bundled dependency notices.
