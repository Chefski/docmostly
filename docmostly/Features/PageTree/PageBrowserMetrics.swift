import SwiftUI

enum PageBrowserMetrics {
    static let rowHeight: CGFloat = 54
    static let iconWidth: CGFloat = 28
    static let headerInsets = EdgeInsets(top: 8, leading: 16, bottom: 4, trailing: 16)
    static let rowInsets = EdgeInsets(top: 8, leading: 16, bottom: 8, trailing: 16)
    static let switchInsets = EdgeInsets(top: 8, leading: 12, bottom: 10, trailing: 12)
    static let railLimit = 12
    static let railHorizontalPadding: CGFloat = 16
    static let railSectionSpacing: CGFloat = 12
    static let railCardSpacing: CGFloat = 12
    static let railCardVerticalOverflowPadding: CGFloat = 24
    static let railCardWidth: CGFloat = 152
    static let railCardHeight: CGFloat = 154
    static let railCardPadding: CGFloat = 16
    static let railCardContentSpacing: CGFloat = 10
    static let railCardCornerRadius: CGFloat = 16
}
