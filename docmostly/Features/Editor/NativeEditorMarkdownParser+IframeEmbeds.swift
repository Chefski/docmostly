...
extension NativeEditorMarkdownParser {
    static func iframeHTMLBlock(
        ...
        while currentIndex < lines.endIndex {
            if containsHTMLClosingTag(in: lines[currentIndex], tagName: "iframe") {
                return (iframeEmbedBlock(source: source), lines.index(after: currentIndex))
            }
            currentIndex = lines.index(after: currentIndex)
        }

        return (iframeEmbedBlock(source: source), lines.index(after: index))
    }
    ...
    static func iframeEmbedMarkdownBlock(from line: String) -> NativeEditorBlock? {
        ...
        return iframeEmbedBlock(source: link.source)
    }
    ...
    private static func iframeEmbedBlock(source: String) -> NativeEditorBlock {
    ...
    private static func isWebEmbedSource(_ source: String) -> Bool {
        guard isWebURL(source) else { return false }

        let path = URLComponents(string: source)?.percentEncodedPath.lowercased() ?? ""
        return path == "/embed" ||
            path.contains("/embed/") ||
            path == "/live-embed" ||
    ...
    private static func isWebURL(_ source: String) -> Bool {
        guard
    ...
    private static func nonEmptyIframeAttribute(_ value: String?) -> String? {
